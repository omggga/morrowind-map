...
DEFAULT_DOCKER_IMAGE = "morrowind-map-openmw:0.51.0-stage45"
...
# The pinned distributions include these manifests but not the Lua resources
# they reference. They only affect UI, actors, and dynamic gameplay systems,
# all of which are deliberately outside the static basemap render contract.
...
def validate_source_inputs(
    ...
    for source in inputs:
        ...
        if not path.is_file():
            raise FileNotFoundError(f"Required OpenMW input is missing: {path}")
        actual = sha256_file(path)
        if actual != source.sha256:
            raise ValueError(
                f"OpenMW input hash mismatch for {source.relative_path}: "
...
def fingerprint_data_directories(source_root: Path) -> dict[str, dict[str, object]]:
    """Fingerprint every loose/archive mount in exact OpenMW override order."""

    audit: dict[str, dict[str, object]] = {}
    for relative_root in DATA_DIRECTORIES:
        root = source_root / relative_root
        if not root.is_dir():
            raise FileNotFoundError(f"Required OpenMW data directory is missing: {root}")
        entries: list[tuple[str, int, str]] = []
        normalized_paths: set[str] = set()
        for path in sorted(root.rglob("*"), key=lambda value: value.as_posix().casefold()):
            if path.is_symlink():
                raise ValueError(f"OpenMW data directories cannot contain symlinks: {path}")
            if not path.is_file():
                continue
            ...
            if normalized in normalized_paths:
                raise ValueError(
                    f"Case-insensitive OpenMW resource collision in {relative_root}: {relative}"
                )
