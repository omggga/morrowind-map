...
- Последний полный аудит: 2026-08-30
- Статус: этапы 0–4.5 завершены; этап 5 в работе — 5.1–5.5 завершены на 100%, Poison Song runtime и `ready` manifest подключены, quality-first V4 renderer/contract/audit подготовлены и прошли Vivec smoke, полный V4 render/publication ещё не запущен; формальная product/browser acceptance 5.6 впереди; generic catalog pipeline готов для адаптации к этапу 6, где остаются Fullrest-specific gaps; этап 7 выполнен частично
...
Status: **in progress; 5.1–5.5 завершены на 100%, включая generic runtime и `ready` manifest; V4 quality pipeline реализован и smoke-проверен, но полный V4 dataset ещё не сгенерирован; остаётся product/browser acceptance 5.6**.
...
3. **5.3 — full Poison basemap: выполнено на 100%.** Все `492` shards дали `3 984` native tiles; после deterministic cross-shard stabilization и полного rebuild lower zoom опубликована sparse lossless WebP pyramid `z0…z7`: `5 464` tiles (`1 480` lower), `1 879 019 148` bytes, inventory `d409e627a75beac2caea56cea35bea132091bc851e1b22edbb4dbe3791f5cd17`. Full audit декодировал каждый tile, точно воспроизвёл `1 480 / 1 480` parents, проверил все `10 467` соседств и `2 571` native cross-shard boundaries, `492` runtime reports, `16` независимых raw seam probes / `32` cells и coordinate error `0 px`. Cross-shard flagged/structural failures — `0/0`, actionable missing resources — `0`; повтор с reused evidence дал тот же audit SHA `4439782ab6e2cdf0d8330168428c4c5550629beaf607f4b9f5507700daf270d5`.
   **V4 visual regeneration: implementation complete, full run pending.** Отдельные roots `poison-song-26.08-v4` и Docker tag `stage5-v4` исключают смешивание с V1. Production v2 применяет single-pass grade `114/102/92` к исходному `544×544` capture после gutter crop и нормализует alpha по правилу `0→0, >0→255`; parent tiles, seam blend, stabilizer v2, audit v3 и publish contract повторяют и проверяют это правило на всей pyramid. Optional `tilePyramid.presentation` отключает browser canvas/CSS correction только для baked V4, сохраняя V1 совместимость и view-only overscale `1.1×`. Реальный Vivec `3×3` smoke рендерит `9/9` native tiles с новым provenance и автоматически собирает воспроизводимый `1536×1536` PNG preview; строгая provenance-проверка отвергает чужие schema/dataset/snapshot/renderer/OpenMW identities. Полный `3 984`-CELL render, finalize, stabilization, audit и content-addressed publication должны быть выполнены последовательностью V4 scripts, включая обязательный deterministic `plan`, перед переключением committed manifest. Короткие generic producer-команды теперь являются V4 aliases; immutable V1 доступен только для отдельной validation.
