...
- Статус: этапы 0–4.5 завершены; этап 5 в работе — 5.1–5.5 завершены на 100%, Poison Song runtime и `ready` manifest подключены, полный quality-first V4 render/finalize/stabilization завершён, исправленный audit и publication ожидают повторного запуска; формальная product/browser acceptance 5.6 впереди; generic catalog pipeline готов для адаптации к этапу 6, где остаются Fullrest-specific gaps; этап 7 выполнен частично
...
Status: **in progress; 5.1–5.5 завершены на 100%, включая generic runtime и `ready` manifest; полный V4 release отрендерен и стабилизирован, исправленный audit и content-addressed publication ожидают запуска; остаётся product/browser acceptance 5.6**.
